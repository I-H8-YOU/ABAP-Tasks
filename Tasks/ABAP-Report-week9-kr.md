...
과제 목표
* Light, Row 색상, Cell색상, Sorting 구현, 필드 카탈로그 구현
...
```
> ALV 리스트에 추가하고 싶은 컬럼 추가

## 1-2. ALV design을 위한 데이터 선언
```abap
...
# 2. InnerJoin 구문을 활용해 새로운 테이블 조인
...
```abap
        ...
        ENDIF.
``` 
>  ID가 1000번인 Row 를 C (default) 5(녹색) 1(깊은색상) 0(반전없이)로 지정
...
```abap
   IF gs_grade-grade = 'D+'. 
    ...
    ENDIF.
```
> D+ Cell을 col_negative (RED) 색으로 지정
